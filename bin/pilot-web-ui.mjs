...
const PID_DIR = resolve(homedir(), '.pilot-web-ui')
...
function startDaemon(port) {
  ...
  if (existing && isRunning(existing)) {
    console.log(`  ✗ pilot-web-ui is already running (PID: ${existing})`)
    console.log(`    Use "pilot-web-ui stop" to stop it first`)
    process.exit(1)
  }
  removePid()
  ...
  setTimeout(() => {
    if (isRunning(child.pid)) {
      console.log(`  ✓ pilot-web-ui started (PID: ${child.pid}, port: ${port})`)
      const url = token
      ...
      console.log('  ✗ Failed to start pilot-web-ui')
      removePid()
      process.exit(1)
    }
  }, 500)
...
function stopDaemon() {
  const pid = getPid()
  if (!pid) {
    console.log('  ✗ pilot-web-ui is not running')
    process.exit(1)
  }

  if (!isRunning(pid)) {
    removePid()
    console.log(`  ✓ pilot-web-ui was not running (cleaned stale PID)`)
    return
  }
  ...
  try {
    ...
    removePid()
    console.log(`  ✓ pilot-web-ui stopped (PID: ${pid})`)
  } catch (err) {
...
function showStatus() {
  ...
  if (pid && isRunning(pid)) {
    console.log(`  ✓ pilot-web-ui is running (PID: ${pid})`)
...
if (['-v', '--version', 'version'].includes(command)) {
  console.log(`pilot-web-ui v${VERSION}`)
  process.exit(0)
}

if (['-h', '--help', 'help'].includes(command)) {
  console.log(`
pilot-web-ui v${VERSION}

Usage: pilot-web-ui <command> [options]
...
function doUpdate() {
  const isWin = process.platform === 'win32'
  const cmd = isWin
    ? 'cmd /c npm install -g pilot-web-ui@latest'
    : 'npm install -g pilot-web-ui@latest'

  console.log('  ⬆ Updating pilot-web-ui...')
