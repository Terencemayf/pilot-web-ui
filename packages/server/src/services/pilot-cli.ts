...
/**
 * List sessions from Pilot CLI (without messages)
 */
export async function listSessions(source?: string, limit?: number): Promise<PilotSession[]> {
  const args = ['sessions', 'export', '-']
  if (source) args.push('--source', source)

  try {
    const { stdout } = await execFileAsync('pilot', args, {
    ...
    for (const line of lines) {
      try {
        const raw: PilotSessionFull = JSON.parse(line)
        let title = raw.title
    ...
    console.error('[Pilot CLI] sessions export failed:', err.message)
    throw new Error(`Failed to list sessions: ${err.message}`)
  }
}

/**
 * Get a single session with messages from Pilot CLI
 */
export async function getSession(id: string): Promise<PilotSession | null> {
  ...
  try {
    ...
    const raw: PilotSessionFull = JSON.parse(lines[0])
    return {
      id: raw.id,
      source: raw.source,
      user_id: raw.user_id,
    ...
    console.error('[Pilot CLI] session export failed:', err.message)
    throw new Error(`Failed to get session: ${err.message}`)
  }
}

/**
 * Delete a session from Pilot CLI
 */
export async function deleteSession(id: string): Promise<boolean> {
  try {
    await execFileAsync('pilot', ['sessions', 'delete', id, '--yes'], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    console.error('[Pilot CLI] session delete failed:', err.message)
    return false
  }
}

/**
 * Rename a session title via Pilot CLI
 */
export async function renameSession(id: string, title: string): Promise<boolean> {
  try {
    await execFileAsync('pilot', ['sessions', 'rename', id, title], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    console.error('[Pilot CLI] session rename failed:', err.message)
    return false
  }
}
...
export async function getVersion(): Promise<string> {
  try {
    const { stdout } = await execFileAsync('pilot', ['--version'], { timeout: 5000, ...execOpts })
    return stdout.trim()
  } catch {
    return ''
  }
...
/**
 * Start Pilot gateway (uses launchd/systemd)
 */
export async function startGateway(): Promise<string> {
  const { stdout, stderr } = await execFileAsync('pilot', ['gateway', 'start'], {
    timeout: 30000,
    ...execOpts,
  })
...
/**
 * Start Pilot gateway in background (for WSL where launchd/systemd is unavailable)
 * Uses "pilot gateway run" as a detached background process
 */
export async function startGatewayBackground(): Promise<number | null> {
  const { spawn } = require('child_process') as typeof import('child_process')
  const child = spawn('pilot', ['gateway', 'run'], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
  })
...
/**
 * Restart Pilot gateway
 */
export async function restartGateway(): Promise<string> {
  const { stdout, stderr } = await execFileAsync('pilot', ['gateway', 'restart'], {
    timeout: 30000,
    ...execOpts,
  })
  return stdout || stderr
...
export async function stopGateway(): Promise<string> {
  const { stdout, stderr } = await execFileAsync('pilot', ['gateway', 'stop'], {
    timeout: 30000,
    ...execOpts,
  })
  return stdout || stderr
...
export async function listLogFiles(): Promise<LogFileInfo[]> {
  try {
    const { stdout } = await execFileAsync('pilot', ['logs', 'list'], {
      timeout: 10000,
      ...execOpts,
    })
    ...
    console.error('[Pilot CLI] logs list failed:', err.message)
    return []
  }
}
...
export async function readLogs(
  ...
  try {
    ...
    console.error('[Pilot CLI] logs read failed:', err.message)
...
export interface PilotProfileDetail {
  name: string
  path: string
  model: string
  provider: string
 ...
 * List all profiles
 */
export async function listProfiles(): Promise<PilotProfile[]> {
  try {
    const { stdout } = await execFileAsync('pilot', ['profile', 'list'], {
      timeout: 10000,
      ...execOpts,
    })
    ...
    console.error('[Pilot CLI] profile list failed:', err.message)
 ...
 * Get profile details
 */
export async function getProfile(name: string): Promise<PilotProfileDetail> {
  try {
    const { stdout } = await execFileAsync('pilot', ['profile', 'show', name], {
      timeout: 10000,
      ...execOpts,
    })
    ...
    console.error('[Pilot CLI] profile show failed:', err.message)
...
export async function createProfile(name: string, clone?: boolean): Promise<string> {
  ...
  try {
    const { stdout, stderr } = await execFileAsync('pilot', args, {
      timeout: 15000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    console.error('[Pilot CLI] profile create failed:', err.message)
...
export async function deleteProfile(name: string): Promise<boolean> {
  try {
    await execFileAsync('pilot', ['profile', 'delete', name, '--yes'], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    console.error('[Pilot CLI] profile delete failed:', err.message)
    return false
  }
}
...
export async function renameProfile(oldName: string, newName: string): Promise<boolean> {
  try {
    await execFileAsync('pilot', ['profile', 'rename', oldName, newName], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    console.error('[Pilot CLI] profile rename failed:', err.message)
    return false
  }
}
...
export async function useProfile(name: string): Promise<string> {
  try {
    const { stdout, stderr } = await execFileAsync('pilot', ['profile', 'use', name], {
      timeout: 10000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    console.error('[Pilot CLI] profile use failed:', err.message)
...
export async function exportProfile(name: string, outputPath?: string): Promise<string> {
  ...
  try {
    ...
    console.error('[Pilot CLI] profile export failed:', err.message)
...
export async function importProfile(archivePath: string, name?: string): Promise<string> {
  ...
  try {
    ...
    console.error('[Pilot CLI] profile import failed:', err.message)
