...
const authPath = resolve(homedir(), '.pilot', 'auth.json')
...
const pilotDir = resolve(homedir(), '.pilot')
...
fsRoutes.get('/api/pilot/skills', async (ctx) => {
  const skillsDir = join(pilotDir, 'skills')
...
fsRoutes.put('/api/pilot/skills/toggle', async (ctx) => {
...
fsRoutes.get('/api/pilot/skills/:category/:skill/files', async (ctx) => {
  const { category, skill } = ctx.params
  const skillDir = join(pilotDir, 'skills', category, skill)
...
fsRoutes.get('/api/pilot/skills/{*path}', async (ctx) => {
  const filePath = (ctx.params as any).path
  const fullPath = resolve(join(pilotDir, 'skills', filePath))

  if (!fullPath.startsWith(join(pilotDir, 'skills'))) {
    ctx.status = 403
...
fsRoutes.get('/api/pilot/memory', async (ctx) => {
  const memoryPath = join(pilotDir, 'memories', 'MEMORY.md')
  const userPath = join(pilotDir, 'memories', 'USER.md')
...
fsRoutes.post('/api/pilot/memory', async (ctx) => {
  ...
  const filePath = join(pilotDir, 'memories', fileName)
...
fsRoutes.get('/api/pilot/available-models', async (ctx) => {
  try {
...
fsRoutes.get('/api/pilot/config/models', async (ctx) => {
  try {
    const config = await readConfigYaml()
...
fsRoutes.put('/api/pilot/config/model', async (ctx) => {
...
fsRoutes.post('/api/pilot/config/providers', async (ctx) => {
...
fsRoutes.delete('/api/pilot/config/providers/:poolKey', async (ctx) => {
