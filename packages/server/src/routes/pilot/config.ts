...
import { restartGateway } from '../../services/pilot-cli'
...
// Matches pilot _apply_env_overrides() in gateway/config.py
const envPlatformMap: Record<string, [string, string]> = {
  ...
  // DingTalk has no _apply_env_overrides entry in pilot;
...
// Write a KEY=value to .env (matching pilot save_env_value behavior)
...
async function saveEnvValue(key: string, value: string): Promise<void> {
  ...
  // Set permissions to 0600 (owner only), matching pilot behavior
...
configRoutes.get('/api/pilot/config', async (ctx) => {
  try {
...
configRoutes.put('/api/pilot/config', async (ctx) => {
...
configRoutes.put('/api/pilot/config/credentials', async (ctx) => {
