...
logRoutes.get('/api/pilot/logs', async (ctx) => {
  const files = await pilotCli.listLogFiles()
  ctx.body = { files }
})
...
logRoutes.get('/api/pilot/logs/:name', async (ctx) => {
  ...
  try {
    const content = await pilotCli.readLogs(logName, lines, level, session, since)
    const rawLines = content.split('\n')

    const entries: (LogEntry | null)[] = []
    for (const line of rawLines) {
      // Skip header lines like "--- ~/.pilot/logs/agent.log (last 100) ---"
