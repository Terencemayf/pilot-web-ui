...
profileRoutes.get('/api/pilot/profiles', async (ctx) => {
  try {
    const profiles = await pilotCli.listProfiles()
    ctx.body = { profiles }
  } catch (err: any) {
    ctx.status = 500
...
profileRoutes.post('/api/pilot/profiles', async (ctx) => {
  ...
  try {
    const output = await pilotCli.createProfile(name, clone)
...
profileRoutes.get('/api/pilot/profiles/:name', async (ctx) => {
  const { name } = ctx.params

  try {
    const profile = await pilotCli.getProfile(name)
    ctx.body = { profile }
  } catch (err: any) {
...
profileRoutes.delete('/api/pilot/profiles/:name', async (ctx) => {
  const { name } = ctx.params
  ...
  try {
    const ok = await pilotCli.deleteProfile(name)
    if (ok) {
...
profileRoutes.post('/api/pilot/profiles/:name/rename', async (ctx) => {
  const { name } = ctx.params
  ...
  try {
    const ok = await pilotCli.renameProfile(name, new_name)
    if (ok) {
      ctx.body = { success: true }
    } else {
      ctx.status = 500
...
profileRoutes.put('/api/pilot/profiles/active', async (ctx) => {
  ...
  try {
    ...
    try { await pilotCli.stopGateway() } catch { }
...
profileRoutes.post('/api/pilot/profiles/:name/export', async (ctx) => {
  const { name } = ctx.params
  const { output } = ctx.request.body as { output?: string }

  try {
    const result = await pilotCli.exportProfile(name, output)
...
profileRoutes.post('/api/pilot/profiles/import', async (ctx) => {
  ...
  try {
    const result = await pilotCli.importProfile(archive, name)
    ctx.body = { success: true, message: result.trim() }
  } catch (err: any) {
    ctx.status = 500
    ctx.body = { error: err.message }
