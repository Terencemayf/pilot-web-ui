...
function buildWsUrl(): string {
  ...
  if (base) {
    return `${wsProtocol}//${new URL(base).host}/api/pilot/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
  }
  ...
  const host = import.meta.env.DEV
    ...
    : location.host;
  return `${wsProtocol}//${host}/api/pilot/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
}
