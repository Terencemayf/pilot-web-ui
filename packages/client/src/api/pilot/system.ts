...
export async function fetchConfigModels(): Promise<ConfigModelsResponse> {
  return request<ConfigModelsResponse>('/api/pilot/config/models')
}

export async function fetchAvailableModels(): Promise<AvailableModelsResponse> {
  return request<AvailableModelsResponse>('/api/pilot/available-models')
}
...
export async function updateDefaultModel(data: {
  ...
  await request('/api/pilot/config/model', {
    method: 'PUT',
...
export async function addCustomProvider(data: CustomProvider): Promise<void> {
  await request('/api/pilot/config/providers', {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function removeCustomProvider(name: string): Promise<void> {
  await request(`/api/pilot/config/providers/${encodeURIComponent(name)}`, {
    method: 'DELETE',
  })
}
