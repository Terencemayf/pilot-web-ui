...
export async function startRun(body: StartRunRequest): Promise<StartRunResponse> {
  return request<StartRunResponse>('/api/pilot/v1/runs', {
    method: 'POST',
...
export function streamRunEvents(
  ...
  const url = `${baseUrl}/api/pilot/v1/runs/${runId}/events${token ? `?token=${encodeURIComponent(token)}` : ''}`
...
export async function fetchModels(): Promise<{ data: Array<{ id: string }> }> {
  return request('/api/pilot/v1/models')
}
