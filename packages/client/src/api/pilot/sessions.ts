...
export async function fetchSession(id: string): Promise<SessionDetail | null> {
  try {
    const res = await request<{ session: SessionDetail }>(`/api/pilot/sessions/${id}`)
    return res.session
  } catch {
    return null
  }
...
export async function deleteSession(id: string): Promise<boolean> {
  try {
    await request(`/api/pilot/sessions/${id}`, { method: 'DELETE' })
    return true
  } catch {
    return false
  }
...
export async function renameSession(id: string, title: string): Promise<boolean> {
  try {
    await request(`/api/pilot/sessions/${id}/rename`, {
      method: 'POST',
