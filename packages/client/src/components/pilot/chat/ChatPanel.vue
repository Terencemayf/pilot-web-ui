...
import { renameSession } from '@/api/pilot/sessions'
import { useChatStore, type Session } from '@/stores/pilot/chat'
...
const collapsedGroups = ref<Set<string>>(new Set(JSON.parse(localStorage.getItem('pilot_collapsed_groups') || '[]')))
...
function toggleGroup(source: string) {
  ...
  localStorage.setItem('pilot_collapsed_groups', JSON.stringify([...collapsedGroups.value]))
}

// Ensure the active session's group is expanded
watch(groupedSessions, (groups) => {
  if (localStorage.getItem('pilot_collapsed_groups') !== null) {
