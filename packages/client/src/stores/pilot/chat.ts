import { startRun, streamRunEvents, type ChatMessage, type RunEvent } from '@/api/pilot/chat'
import { deleteSession as deleteSessionApi, fetchSession, fetchSessions, type PilotMessage, type SessionSummary } from '@/api/pilot/sessions'
...
function mapPilotMessages(msgs: PilotMessage[]): Message[] {
...
function mapPilotSession(s: SessionSummary): Session {
  return {
    id: s.id,
    title: s.title || '',
...
export const useChatStore = defineStore('chat', () => {
  const STORAGE_KEY = 'pilot_active_session'
  ...
  async function loadSessions() {
    ...
    try {
      ...
      sessions.value = list.map(mapPilotSession)
    ...
    if (activeSession.value && activeSession.value.messages.length === 0) {
      ...
      try {
        ...
        if (detail && detail.messages) {
          const mapped = mapPilotMessages(detail.messages)
          ...
          // Update title: use Pilot title, or fallback to first user message
          if (detail.title) {
  ...
  async function switchSessionModel(modelId: string, provider?: string) {
    ...
    // If provider changed, update global config too (Pilot requires it)
    if (provider) {
